...
- Format: `01PL-216061-125-C22S-H`，管道号可带字母后缀如 `216061A`
- Output: `01PL-216061`
- Pattern: `(\d{2}[A-Za-z]{2,4})-(\d{5,7}[A-Z]?)-(\d{2,4})-([A-Z0-9]{2,6})-([A-Z]{1,2})`
- 注意：介质代码支持大小写混合（如 `AcS` 浓硫酸）

### 金昱元项目 (Jinyuyuan Project)
- Format: `01CSL03203-150-C1N-H`，管道号可带字母后缀如 `04001A`
- Output: `01CSL03203`
- Pattern: `(\d{2}[A-Z]{1,4}\d{5,7}[A-Z]?)-(\d{2,4})-([A-Z0-9]{2,6})-([A-Z]{1,2})`
- 注意：第一段将装置号、介质代码、管道号合并，无内部连字符；介质代码支持1-4字母（含单字母如 `H`、`D`）
