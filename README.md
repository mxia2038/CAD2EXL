...
- 🏗️ **多项目标准支持** - 支持巨化项目和乌兹项目两种管道编号标准，可轻松扩展
- 🔍 **智能管道号识别** - 升级的正则表达式，支持更灵活的介质代码格式
- 📊 **Excel报告生成** - 生成包含管道详细信息的Excel报告
- 🧪 **智能相态判断** - 基于介质名称自动判断液相/气相
- 🎯 **优化用户界面** - 下拉式项目选择器，动态格式示例显示
- 🖱️ **增强拖拽功能** - 支持包含空格的文件名，完善的视觉反馈
...
1. 启动程序
2. **选择项目类型** - 从下拉菜单中选择：
   - 🏗️ **巨化项目**：传统格式 `4101BRR-02457-200-03CBMB1-H`
   - 🌍 **乌兹项目**：新格式 `PA-2001002A-100-C1C-N`
3. **选择文件** - 支持三种方式：
   - 🖱️ **拖拽方式**：直接将文件拖拽到对应的输入框区域
   - 📋 **最近文件**：从下拉菜单中选择最近使用的文件
   - 📁 **浏览按钮**：传统的文件选择对话框
4. 选择DWG文件（自动识别.dwg格式）
5. 选择介质代码Excel文件（自动识别.xlsx/.xls格式）
6. 指定输出文件位置
7. 点击"开始提取"
8. 等待处理完成
...
### 🌍 乌兹项目格式
```
介质代码-管道号-管径-管道等级-保温类型
示例: PA-2001002A-100-C1C-N
输出: PA-2001002A

格式说明:
- 介质代码: 1-4位字母数字 (PA, BW等)
- 管道号: 4-8位字母数字 (2001002A等)
- 管径: 2-4位数字 (100, 150, 200等)
- 管道等级: 1-4位字母数字 (C1C, D2D等)
- 保温类型: 1-2位字母数字 (N, H, C等)
```
